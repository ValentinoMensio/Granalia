...
class PostgresMigrationMixin(PostgresRepositoryProtocol):
    ...
    def _ensure_arca_fiscal_base(self, *, connection) -> None:
        ...
        connection.execute(
            text(
                """
                CREATE TABLE IF NOT EXISTS arca_iva_rates (
        ...
        invoice_fields = [
            ("internal_number", "BIGINT"),
            ("fiscal_kind", "VARCHAR(20) NOT NULL DEFAULT 'internal'"),
            ("fiscal_status", "VARCHAR(30) NOT NULL DEFAULT 'draft'"),
            ("arca_environment", "VARCHAR(20)"),
            ("arca_cbte_tipo", "INTEGER"),
            ("arca_point_of_sale", "INTEGER"),
            ("arca_invoice_number", "BIGINT"),
            ("arca_cae", "VARCHAR(32)"),
            ("arca_cae_due_date", "DATE"),
            ("arca_authorized_at", "TIMESTAMPTZ"),
            ("arca_result", "VARCHAR(20)"),
            ("arca_observations_json", "JSONB"),
            ("arca_errors_json", "JSONB"),
            ("arca_request_id", "BIGINT"),
        ...
        for column_name, definition in invoice_fields:
            if not self._column_exists(connection, "invoices", column_name):
                connection.execute(text(f"ALTER TABLE invoices ADD COLUMN {column_name} {definition}"))
        connection.execute(text("UPDATE invoices SET internal_number = COALESCE(internal_number, invoice_number)"))
        connection.execute(text("UPDATE invoices SET fiscal_kind = CASE WHEN declared THEN 'fiscal' ELSE 'internal' END WHERE fiscal_kind IS NULL OR fiscal_kind = ''"))
        connection.execute(text("UPDATE invoices SET fiscal_status = 'draft' WHERE fiscal_status IS NULL OR fiscal_status = ''"))
        connection.execute(text("ALTER TABLE invoices ALTER COLUMN internal_number SET NOT NULL"))

        if self._table_exists(connection, "product_offerings"):
            offering_fields = [
                ("iva_rate_id", "INTEGER"),
                ("iva_rate_percent", "NUMERIC(6, 3)"),
            ]
            for column_name, definition in offering_fields:
                if not self._column_exists(connection, "product_offerings", column_name):
                    connection.execute(text(f"ALTER TABLE product_offerings ADD COLUMN {column_name} {definition}"))
        ...
        if self._table_exists(connection, "invoice_items"):
            item_fields = [
                ...
                ("gross_unit_price", "NUMERIC(14, 4)"),
                ("tax_amount", "NUMERIC(14, 2)"),
                ("line_net_amount", "NUMERIC(14, 2)"),
        ...
        connection.execute(
            text(
                ...
                CREATE TABLE IF NOT EXISTS invoice_tax_breakdown (
                    ...
                    arca_iva_id INTEGER NOT NULL REFERENCES arca_iva_rates(arca_id) ON DELETE RESTRICT,
                    iva_rate_percent NUMERIC(6, 3) NOT NULL,
                    base_amount NUMERIC(14, 2) NOT NULL,
                    tax_amount NUMERIC(14, 2) NOT NULL,
                    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
                    UNIQUE(invoice_id, arca_iva_id)
                )
                """
            )
        )
        connection.execute(
            text(
                """
                CREATE TABLE IF NOT EXISTS arca_requests (
                    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
