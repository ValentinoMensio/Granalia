...
function buildInvoicePayload(form, currentCustomer) {
  ...
  return {
    order: {
      ...
      fiscal_kind: form.fiscalKind || (form.declared ? 'fiscal' : 'internal'),
      declared: (form.fiscalKind || (form.declared ? 'fiscal' : 'internal')) === 'fiscal',
      secondary_line: form.secondaryLine,
      transport: form.transport,
      notes: splitNotes(form.notes),
      items: form.items
...
function buildFormFromInvoiceDetail(invoiceDetail, customers) {
  ...
  return {
    ...
    fiscalKind: invoiceDetail.fiscal_kind || (invoiceDetail.declared ? 'fiscal' : 'internal'),
